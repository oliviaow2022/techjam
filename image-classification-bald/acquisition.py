...
class DiversitySampling(Acquirer):
    @staticmethod
    def score(model, x):
        with torch.no_grad():
            # Extract features using the model
            features = model(x, return_logits=True)
    ...
    def select_batch(self, model, pool_data):
        ...
        # Extract features for all data points in the pool
        all_features = []
        for data, _ in pool_loader:
            features = self.score(model, data.to(self.device))
            all_features.append(features)
        all_features = torch.cat(all_features, dim=0)

        # Select the first point randomly
        selected_indices = [np.random.choice(len(pool_data))]
        selected_features = all_features[selected_indices]

        for _ in range(self.batch_size - 1):
            # Calculate the distance between each point in the pool and the selected points
            distances = torch.cdist(all_features, selected_features)
            min_distances = distances.min(dim=1)[0]
            # Select the point with the maximum minimum distance
            next_index = min_distances.argmax().item()
            selected_indices.append(next_index)
            selected_features = torch.cat([selected_features, all_features[next_index].unsqueeze(0)], dim=0)

        best_global_indices = np.array(pool_data.indices)[selected_indices]
        return best_global_indices

class LeastConfidence(Acquirer):
    def __init__(self, pool_data, device):
        super(LeastConfidence, self).__init__(pool_data, device)

    @staticmethod
    def score(model, x):
        """
        Compute the least confidence score for samples.

        :param model: The model used for prediction
        :param x: Input data
        :return: Scores indicating the least confidence
        """
        with torch.no_grad():
            ...
            # Compute the least confidence score as 1 - max probability
            least_confidence_scores = 1 - max_probabilities
            
            return least_confidence_scores
            
class Entropy(Acquirer):
    def __init__(self, pool_data, device):
        super(Entropy, self).__init__(pool_data, device)
    ...
    def entropy(p):
        ...
        :param p: Tensor of probabilities (batch_size, num_classes)
        :return: Entropy of the distribution
        """
        p = p + 1e-10  # To avoid log(0) which is undefined
