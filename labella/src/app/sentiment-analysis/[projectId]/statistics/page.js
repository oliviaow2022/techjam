...
export default function SentimentAnalysisStatistics({ params }) {
  ...
  const handleDownloadDataset = async (projectId) => {
    try {
      ...
      const response = await apiClient.get(process.env.NEXT_PUBLIC_API_ENDPOINT + `/dataset/${projectId}/download`, {
